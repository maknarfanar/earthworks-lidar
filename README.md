...
Weights for the baseline model and the baseline model+HPF are included in the tiles_and_weights folder. See the code for more information. The Jupyter Notebooks contain sections that can be uncommented to load the weights. Make certain you assign the CAE architecture to "model" before loading weights. 
